    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors ${tab === 'profil' ? 'bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
        >
        ...
        <button
          onClick={() => setTab('umum')}
          className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors ${tab === 'umum' ? 'bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
        >
        ...
        <form onSubmit={handleProfileSubmit} className="bg-surface rounded-xl border border-border p-6 space-y-5">
          ...
          <div className="flex items-center gap-4 p-4 bg-bg-subtle rounded-lg">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-text-primary">Logo Desa</p>
              <div className="flex items-center gap-2 mt-1">
                <input type="file" accept="image/*" onChange={(e) => setLogoFile(e.target.files?.[0] || null)}
                  className="text-sm text-text-secondary file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-primary-50 file:text-primary-700" />
                {logoFile && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-text-primary mb-1">Nama Desa *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Kabupaten *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Nama Kepala Desa</label>
              <input type="text" value={profileForm.nama_kepala_desa}
                onChange={(e) => setProfileForm({ ...profileForm, nama_kepala_desa: e.target.value })}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Telp</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Luas Wilayah (km²)</label>
              ...
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Jumlah Dusun</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-text-primary mb-1">Misi</label>
              <textarea value={profileForm.misi}
                onChange={(e) => setProfileForm({ ...profileForm, misi: e.target.value })}
                rows={3} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-text-primary mb-1">Sejarah Desa</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-text-primary mb-1">Sambutan Kepala Desa</label>
      ...
      {tab === 'umum' && (
        <form onSubmit={handleSettingsSubmit} className="bg-surface rounded-xl border border-border p-6 space-y-4">
          {Object.keys(settingsForm).length === 0 ? (
            <p className="text-text-muted text-sm">Belum ada pengaturan.</p>
          ) : (
            Object.entries(settingsForm).map(([key, value]) => (
              <div key={key}>
                <label className="block text-sm font-medium text-text-primary mb-1 capitalize">{key.replace(/_/g, ' ')}</label>

  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/admin/surat/jenis-surat')} className="text-fg-muted hover:text-fg-secondary">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-fg">{isEdit ? 'Edit' : 'Tambah'} Jenis Surat</h1>
          <p className="text-fg-secondary text-sm">Kelola template dan persyaratan surat</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-surface rounded-xl border border-border p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-fg mb-1">Kode *</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-fg mb-1">Estimasi (hari kerja)</label>
            <input
              type="number" min={1}
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-fg mb-1">Deskripsi</label>
            <textarea
        ...
        {/* Persyaratan */}
        <div>
          <label className="block text-sm font-medium text-fg mb-2">Persyaratan</label>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={addSyarat}
              className="px-3 py-2 bg-gray-100 text-fg text-sm rounded-lg hover:bg-gray-200 transition-colors">
        ...
        {/* Template Konten */}
        <div>
          <label className="block text-sm font-medium text-fg mb-1">Template Konten Surat</label>
          <div className="flex flex-wrap gap-1.5 mb-2">
            {PLACEHOLDERS.map((p) => (
              <button
                ...
                className="px-2 py-0.5 bg-gray-100 text-fg-secondary text-xs rounded hover:bg-primary-100 hover:text-primary-700 transition-colors"
              >
                {p}
              </button>
            ))}
          </div>
          <p className="text-xs text-fg-muted mb-1">Klik placeholder untuk menyisipkan ke posisi kursor</p>
          <textarea
            ref={templateRef}

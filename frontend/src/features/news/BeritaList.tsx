  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-fg">Berita</h1>
          <p className="text-fg-secondary text-sm mt-1">Total: {data?.total || 0} berita</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-subtle border-b border-border">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-fg-secondary uppercase">Kategori</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-fg-secondary uppercase">Status</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-fg-secondary uppercase">Tanggal</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="text-center py-8 text-fg-muted">Belum ada berita</td></tr>
              ) : (
                items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-subtle transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${item.status === 'published' ? 'bg-green-50 text-green-700' : 'bg-subtle text-fg-secondary'}`}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="px-3 py-1.5 text-sm border border-border rounded-lg disabled:opacity-50 hover:bg-subtle">Sebelumnya</button>
              <button onClick={() => setPage(p => p + 1)} disabled={page >= (data.last_page || 1)}
                className="px-3 py-1.5 text-sm border border-border rounded-lg disabled:opacity-50 hover:bg-subtle">Selanjutnya</button>
            </div>
          </div>
        )}
      </div>
